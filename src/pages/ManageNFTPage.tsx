...
import { NFT_COLLECTION_IMAGES, NFTCollectionContract, getExplorerUrl, getNativeTokenLabel } from '@/config';
...
const ManageNFTPage: React.FC = () => {
  ...
  const contractBalanceLabel = contractBalance
    ? `${Number(contractBalance.formatted).toLocaleString(undefined, { maximumFractionDigits: 4 })} ${
        contractBalance.symbol ?? nativeToken
      }`
    : `0 ${nativeToken}`;
        ...
        <div className="glass-card rounded-3xl p-5">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="font-display text-display-sm text-ink">{formatEther(collection.mintPrice)} {nativeToken}</p>
            </div>
            <div className="space-y-1">
              <p className="text-body-sm text-ink-muted">Whitelist Price</p>
              <p className="font-display text-display-sm text-ink">
                {collection.whitelistEnabled ? `${formatEther(collection.whitelistPrice)} ${nativeToken}` : 'Disabled'}
              </p>
            </div>
            <div className="space-y-1">
      ...
      {isOwner && (
        <>
          ...
          <motion.section variants={itemVariants} className="glass-card rounded-3xl p-6 space-y-5">
            ...
            <div className="space-y-2">
              <label className="text-body-sm text-ink-muted font-medium">Amount ({nativeToken})</label>
              <input
                value={withdrawInput}
